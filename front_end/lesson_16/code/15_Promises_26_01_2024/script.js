...
stats.onclick = () => {
  if (stock.items.length) {
    const itemsQuantity = stock.items.length; // Количество позиций или длина массива
    const totalCost = stock.totalCost;
    const totalQuantity = stock.items.reduce((acc, item) => acc + item.quantity, 0);

    // const maxPrice = stock.items.reduce((acc, item) => acc >= item.price ? acc : item.price, 0);
    
    // const maxPrice = stock.items.sort((a, b) => a.price - b.price)[itemsQuantity - 1]; // Сортировка по цене от меньшей к большей
    // const maxPrice = stock.items.sort((a, b) => b.price - a.price)[0]; // Сортировка по цене от большей к меньшей

    const maxPrice = Math.max(...stock.items.map((e) => e.price));
    const minPrice = Math.min(...stock.items.map((e) => e.price));
